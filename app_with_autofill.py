...
presets = {
    "Regular": ["delhi", "underground", "iii", 125, 210, 32, 18, 5, 3, "typ-a"],
    "Terminal": ["mumbai", "elevated", "ii", 145, 180, 30, 16, 4, 2, "typ-b"],
    "Interchange": ["chennai", "underground", "iv", 160, 240, 36, 20, 6, 4, "typ-c"],
    "Custom": ["" for _ in feature_columns]
}

st.sidebar.header("🧮 Input Parameters")
inputs = {}
for i, feature in enumerate(feature_columns):
    inputs[feature] = st.sidebar.text_input(label=feature, value=str(presets[station_type][i]))

def predict(inputs):
    df_input = pd.DataFrame([inputs])
    df_input = df_input.astype({col: "float" if col not in ["city", "metro_type", "seismic_zone", "station_typology"] else "object" for col in df_input.columns})
...
if uploaded_file is not None:
    try:
        ...
        df = df.astype({col: "float" if col not in ["city", "metro_type", "seismic_zone", "station_typology"] else "object" for col in df.columns})
